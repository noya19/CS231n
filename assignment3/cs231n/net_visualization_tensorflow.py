...
def compute_saliency_maps(X, y, model):
    ...
    N = X.shape[0]
    x = tf.constant(X, dtype=tf.float32)     #have to covert the input to a tensor of type=float32,
                                             #or the gradient printed will be of none type
    #Step1
    with tf.GradientTape() as t:
      t.watch(x)
      correct_scores = tf.gather_nd(model.call(x), tf.stack((tf.range(N), y), axis=1))
    #Step3
    dL_dimage=t.gradient(correct_scores,x)
    #Step-4:compute the saliency map
    saliency = np.abs(dL_dimage).max(axis=3)  #take the absolute of the gradients
...
def make_fooling_image(X, target_y, model):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)****
    X_fooling = tf.constant(X_fooling, dtype=tf.float32)
    X_res = X_fooling
    for i in range(100):
        with tf.GradientTape() as t:
          t.watch(X_res)
          scores = model.call(X_res)
          print("Iter %d , loss %lf , predicted %d - target %d" % (i, scores[0][target_y], np.argmax(scores[0]), target_y) )
          if np.argmax(scores[0]) == target_y:
              break;
          g = t.gradient(scores[0][target_y], X_res)
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as t:
      t.watch(X1)
      scores = model.call(X1)
      reg_term = l2_reg * tf.nn.l2_loss(X1)
      loss = scores[0][target_y]-reg_term
      g = t.gradient(loss, X1)
      dx = learning_rate * g / tf.norm(g,ord=2)
      X1 =X1 + dx
    X = X1      
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
